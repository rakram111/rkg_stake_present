...
contract TronBeast {
    ...
    uint8[] public tbt_offer ;  
    uint8[] public next_deposit ;  

    uint256 constant public one_day = 600 ; // 1 days 
    ...
    constructor(address payable _owner, 
        ...
        pack_values.push(10 trx);  // 100 trx
        pack_values.push(50 trx);  // 10000 trx
        pack_values.push(100 trx); // 100000 trx
        pack_values.push(150 trx); // 500000 trx
        pack_values.push(200 trx); // 1000000 trx
        ...
        for(uint256 i = pack_values.length; i >= 0; i--){
    ...
    function _deposit(address _addr, uint256 _amount) private {
        ...
        if(_amount >= tbt_min_deposit){ // 1000 trx | 50 trx
            if( total_users <= tenk_users){ 
                ...
                tbt_amount = _amount/(2*tbt_price/10);  // 2000 trx | 40 trx per token 
            }  
    ...
    function withdraw(address payable _addr) external {
        ...
        require(block.timestamp > users[msg.sender].deposit_time + users[msg.sender].payout_time, "Cannot withdraw now"); 

        uint256 to_payout = 7*users[msg.sender].max_payout/10;
        uint256 to_tbt;
        uint256 tbt_amount;
        users2[msg.sender].locked_balance = 3*users[msg.sender].max_payout/10;

        for (uint8 i = 0; i < tbt_offer.length; i++) {
            if(users[msg.sender].my_num == i){
                to_tbt = to_payout*tbt_offer[i]/100;
                if( total_users <= tenk_users){ 
                    tbt_amount = to_tbt/(tbt_price/10);  // 1000 trx | 20 trx per token
                } else {
                    tbt_amount = to_tbt/(2*tbt_price/10);  // 2000 trx | 40 trx per token 
                }  
                tbt.transfer(msg.sender,tbt_amount*100000); // token transfer
                users2[_addr].tbt_from_withdrawal += tbt_amount*100000 ;
                users2[_addr].total_tbt += tbt_amount*100000 ;
                to_payout -= to_tbt ;
                users2[msg.sender].next_min_deposit = to_payout*next_deposit[i]/100;
                break; 
            } 
        }
        ...
        users2[msg.sender].total_payouts += to_payout;
    ...
    */ 
    ...
    function payoutTimeRemaining() external view returns (uint256){ 
        uint256 time_remaining = users[msg.sender].deposit_time + users[msg.sender].payout_time - block.timestamp ;
        return time_remaining ;
    }

    function userInfo(address _addr) view external returns(address upline, uint40 deposit_time, uint256 deposit_amount,  uint256 direct_bonus  , uint256 user_status) {
        return (users[_addr].upline, users[_addr].deposit_time, users[_addr].deposit_amount,    users[_addr].direct_bonus , users[_addr].isActive );
    }
    ...
    function userInfo2(address _addr) view external returns( uint256 temp_directs_count, uint256 next_deposit1, uint256 tbt_offer1, uint256 temp_direct_count, uint256 total_receivable ) {
        return ( users[_addr].temp_directs_count, users2[_addr].next_min_deposit, 
        7*users[_addr].max_payout*tbt_offer[users[_addr].my_num]/1000,
        users[_addr].temp_directs_count, users[_addr].max_payout);
    }
